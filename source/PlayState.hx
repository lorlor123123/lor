	...
	override public function create()
	{
		...
		switch (SONG.song.toLowerCase())
		{
			...
			case 'flame' | 'stain':
				...
				curStage = 'philip';
				var bg:BGSprite = new BGSprite('back', -600, -400, 0.9, 0.9);
				add(bg);

				var stageFront:BGSprite = new BGSprite('stage1', -650, 265, 0.9, 0.9);
				stageFront.setGraphicSize(Std.int(stageFront.width * 1.0));
				stageFront.updateHitbox();
				add(stageFront);

				var micw:BGSprite = new BGSprite('mic/mic_with_weapon', 385, 0, 1, 1);
				micw.updateHitbox();
				add(micw);
		...
		//micw = new BGSprite('mic/mic_with_weapon', 385, 0, 1, 1);
				...
				case 4: //Hurt note
					...
					if(!boyfriend.stunned)
					{
						...
						if(!endingSong)
						{
							...
							else health -= 0.56; //0.06 + 0.04 = -0.1 (-5%) of HP if you hit a hurt sustain note
